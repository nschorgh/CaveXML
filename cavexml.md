...
The aggregate length of the cave passages in meters, usually obtained by summing the lengths of all surveyed distances. Length differs from horizontal extent. The length must be rounded to the nearest integer. The unit must be meters, not km or feet. Symbols ~ and > are allowed in front of the number.   
...
-   CaveXML does not require a catalog number for each record, because it is designed to work with a distributed collection of databases rather than with a single central database.
